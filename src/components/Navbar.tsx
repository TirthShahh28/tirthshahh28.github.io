  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <a
          href="#hero"
          className="text-lg font-bold tracking-tight font-mono text-white"
        >
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="transition-colors text-sm px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5"
            >
              {link.label}
            </a>
          ))}
      ...
      {mobileOpen && (
        <div className="md:hidden backdrop-blur-xl border-t bg-[#0a0a0a]/95 border-white/5">
          <div className="flex flex-col items-center gap-2 py-6 px-6">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="transition-colors w-full text-center py-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5"
